                    ...
                    <div className="flex-column">
                        ...
                        <Grid item className="buyerMemberSignUpWrapper__content right" item xs={12} lg={6}>
                            {BuyerMemberFormRight.map((item, index) => {
                                ...
                                const radioButtonSelectionRight = item.length > 0 && <div className="selectionField__content"><label className="signupInputFields__label">{item[0].title}</label> <div className="subFields">{item.map((field, fieldindex) => <div key={field + fieldindex}>{!field.title && <div className="subFieldsContent"><Field label={field.label} value={field.value} type={field.type} name={field.name} component='input' /><label>{field.label}</label> </div>} </div>)}</div></div>;
